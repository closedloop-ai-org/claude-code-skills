...
2. **Compile the request into a rule IR.** Two threshold shapes are supported — a **threshold over product insights** (`subject: "insight"`) or over **strategic-intelligence signals incl. churn** (`subject: "signal"`):
   ...
   - **Affect / "sentiment"**: there is NO sentiment field (it's dead). For "angry / frustrated / upset / unhappy / negative" use `insight.emotion` — a FIXED vocabulary `["neutral","frustrated","hopeful","anxious","urgent","resigned","relieved"]` — e.g. `{ "predicate": "insight.emotion", "op": "in", "value": ["frustrated","anxious"] }`. `insight.frustration_score` (0-1) skews low in practice — do NOT assume `>= 0.8`; only use a numeric frustration threshold the user explicitly states.
